...
### Exercise 1: When does migration break cooperation?

In this problem we want to understand the effect of migration on cooperation prevalence. Implement "migration" as follows:

1. Add another model parameter: `migration_rate`.
1. Every agent might migrate. You could iterate through each agent and see `if (runif() < migration_rate)`. A more efficient option might be to use, e.g., `n_migrating <- round((migration_rate * N) / 2.0)`, which can then be used to obtain `migrating_agents_idxs <- sample(1:N, n_migrating)` on each time step, then select a partner for each selected agent from the set of all agents not already set to migrate, which can be found using `setdiff(1:N, migrating_agent_idxs)`.
1. Set `behavior_current` for each to be that of the other, i.e., exchange `behavior_current`. This can be best done after the `iterate_learning_model` call at the end of the `coop_model_step` definition in `R/model.R`.

If all goes well you should get a figure that looks like Figure 6.6 in Ch. 6 of Smaldino's (2023) _Modeling Social Behavior_ [@Smaldino2023_Ch6Cooperation].

### Exercise 2: Disastrous defection
...
### Exercise 3: The shape of cooperation
...
### Exercise 5: Interconnected communities
